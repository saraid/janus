...
class Model extends Base
  ...
  _emitChange: (key, newValue, oldValue) ->
    ...
    if newValue instanceof Reference and this.attribute(key)?.transient isnt true
      ...
      this._watchSubreferences$ ?= do =>
        watchReference = (ref) =>
          resolveReference = (model) =>
            if model instanceof Model and ref in uniqSubreferences.list
        ...
        watchReference(ref) for ref in uniqSubreferences.list
